...
beforeAll(async function (){
  try {
    ...
    insertedUser = await user.insertOne({
      username: 'Koby_Wolf',
      password: 'BEWuTUlBJeQ8PKj',
      email: 'Gonzalo80@hotmail.com',
      name: 'Helen Haag',
      shippingAddress: [
        {
          _id: false,
      ...
      wishlist: [ {
        _id: false,
        productId: 'demand-driven' 
      } ],
      userType: 719,
      mobileNo: '(241) 283-5109',
      resetPasswordLink: {},
      loginRetryLimit: 139,
      loginReactiveTime: '2023-04-22T03:25:04.886Z',
      id: '62ff70abb304ad8ebcaba744'
    });
  }
  catch (error) {
...
describe('POST /register -> if email and username is given', () => {
  test('should register a user', async () => {
    let registeredUser = await request(app)
      ...
      .send({
        'username':'Mathew_Gibson',
        'password':'wi98LF92wk321EH',
        'email':'Ralph.Grady@hotmail.com',
        'name':'Johnathan Bergstrom',
        'shippingAddress':[{
          '_id':false,
          'pincode':'foreground',
          'address1':'wireless',
...
describe('POST /forgot-password -> if email passed from request body is valid and OTP sent successfully', () => {
  test('should return success message', async () => {
    let user = await request(app)
      .post('/client/auth/forgot-password')
      .send({ 'email':'Ralph.Grady@hotmail.com', });
